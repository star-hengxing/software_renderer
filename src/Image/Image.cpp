...
std::optional<Image> Image::read(std::string_view filename)
{
	...
	{
		memcpy(&image.get().data()->r, data, w * h * 4);
		result = std::move(image);
	}
...
void Image::write(std::string_view filename, Image::Type type, const u8* data, usize width, usize height)
{
...
void Image::write(std::string_view filename, Type type) const
{
